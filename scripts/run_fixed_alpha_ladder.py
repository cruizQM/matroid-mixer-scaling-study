"""Fixed-cost_alpha=0.01 baseline for the escalating-realism ladder,
regenerated under CURRENT code (after the never-fire fix) -- the
original adaptive-vs-fixed comparison was computed before that fix and
is stale (see docs). Same conditions/sizes/seeds as
run_cost_aware_scaling_ladder.py; only `adaptive=False, cost_alpha=0.01`
differs.

Writes results/fixed_alpha_ladder_results.csv and _summary.csv.
"""
...
from run_cost_aware_scaling_ladder import CONDITIONS, N_NODES_RANGE, SEEDS_PER_SIZE, WALK_STEPS, MAX_WITNESS_SIZE, EXACT_SEARCH_MAX_SIZE, N_STARTING_TREES, UNSAFE_TOL, BETA

RESULTS_DIR = Path(__file__).resolve().parent.parent / "results"
CSV_PATH = RESULTS_DIR / "fixed_alpha_ladder_results.csv"
SUMMARY_CSV_PATH = RESULTS_DIR / "fixed_alpha_ladder_summary.csv"
...
def run_one(condition: str, n_nodes: int, seed: int) -> dict:
    ...
    truncated = build_truncated_witness_mixer(
        ...
        adaptive=False, cost_alpha=0.01,
    )
    ...
    return {
        ...
        "n_qubits": graph.n_edges, "cx_count": cx_count, "depth": depth,
